...
def main():
    ...
    label_mapping = {'apple': 0, 'banana': 1, 'blackgram': 2, 'chickpea': 3, 'coconut': 4, 'coffee': 5,
                 ...
                 'papaya': 17, 'pigeonpeas': 18, 'pomegranate': 19, 'rice': 20, 'watermelon': 21,'groundnuts': 22,}
    col = st.columns(1)[0]
    ...
    with col:
        ...
        humidity = st.number_input("Humidity in %", 0.0, 150.0)
...
# Get the corresponding category label
            predicted_label = next(key for key, value in label_mapping.items() if key == predicted_label_index)
